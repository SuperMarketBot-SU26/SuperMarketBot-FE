...
export const getLatestMap = async ({ floorId } = {}) => {
  const res = await editorClient.get('/v1/maps/latest', { params: { floorId } })
  return res.data
}

/** POST /v1/maps/sync */
export const syncMap = async (payload) => {
  const res = await editorClient.post('/v1/maps/sync', payload)
  return res.data
}

/** POST /v1/maps/{mapId}/upload-image (multipart) */
export const uploadMapImage = async (mapId, formData) => {
  const res = await editorClient.post(`/v1/maps/${mapId}/upload-image`, formData, {
...
export const getRoutes = async ({ mapId, zoneId, routeType } = {}) => {
  const res = await editorClient.get('/v1/routes', { params: { mapId, zoneId, routeType } })
  return res.data ?? []
}

/** GET /v1/routes/{routeId} */
export const getRoute = async (routeId) => {
  const res = await editorClient.get(`/v1/routes/${routeId}`)
  return res.data
}

/** POST /v1/routes */
export const createRoute = async (payload) => {
  const res = await editorClient.post('/v1/routes', payload)
  return res.data
}

/** PUT /v1/routes/{routeId} */
export const updateRoute = async (routeId, payload) => {
  const res = await editorClient.put(`/v1/routes/${routeId}`, payload)
  return res.data
}

/** DELETE /v1/routes/{routeId} */
export const deleteRoute = async (routeId) => {
  const res = await editorClient.delete(`/v1/routes/${routeId}`)
