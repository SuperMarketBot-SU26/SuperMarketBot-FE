...
/* maintain older export name for compatibility */
export const cancelNavigation = (robotCode) => cancelRobotNavigation(robotCode)
...
export const optimizeShoppingRoute = async (payload) => {
  const res = await client.post(`${V1_NAV_ENDPOINT}/optimize-shopping-route`, payload)
  return res.data
}

// 9. Sync Standard Zone Names
export const syncStandardZoneNames = async () => {
  const res = await client.post(`${V1_NAV_ENDPOINT}/sync-standard-zone-names`)
  return res.data
}
