  ...
  if (variant === 'compact') {
    return (
      <div className={`flex flex-col gap-1 ${className}`}>
        {/* Pin Tổng */}
        <div className="flex items-center gap-1.5" title={`Pin tổng hợp: ${overall ?? '?'}%`}>
          <Icon
            name={getBatteryIcon(overall, deviceIsCharging)}
            className={`text-[16px] ${overall > 50 ? 'text-emerald-500' : overall >= 20 ? 'text-amber-500' : 'text-rose-500'}`}
          />
          <div className="relative h-2 w-10 overflow-hidden rounded-full bg-smb-surface-container-high border border-smb-outline-variant/60">
            <div
              className={`h-full transition-all duration-300 ${overall > 50 ? 'bg-emerald-500' : overall >= 20 ? 'bg-amber-500' : 'bg-rose-500'}`}
        ...
        <div className="flex items-center gap-2 text-[10px] text-smb-on-surface-variant/80 font-medium">
          <span className="inline-flex items-center gap-0.5" title="Pin máy tính bảng (Kiosk App)">
            <Icon name="smartphone" className="text-[11px] text-blue-500" />
            <span className="tabular-nums">{devBat != null ? `${devBat}%` : '—'}</span>
            {deviceIsCharging && <Icon name="bolt" className="text-[10px] text-amber-500" />}
          </span>
          ...
          <span className="inline-flex items-center gap-0.5" title="Pin động cơ xe (Giả lập ESP32)">
            ...
            <span className="tabular-nums">{espBat != null ? `${espBat}%` : '—'}</span>
    ...
    <div className={`rounded-xl border border-smb-outline-variant/50 bg-smb-surface-container-lowest/80 p-3 shadow-xs ${className}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div className={`flex size-8 shrink-0 items-center justify-center rounded-lg ${overall > 50 ? 'bg-emerald-500/15 text-emerald-600 dark:text-emerald-400' : overall >= 20 ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400' : 'bg-rose-500/15 text-rose-600 dark:text-rose-400'}`}>
          ...
          <div>
            ...
            <p className="text-[10px] text-smb-on-surface-variant/80">
              min(Tablet, ESP32) · {overall != null ? (overall > 50 ? 'Dung lượng an toàn' : overall >= 20 ? 'Mức trung bình' : 'Cần về trạm sạc') : 'Chưa có dữ liệu'}
            </p>
          </div>
        </div>
      ...
      <div className="relative h-2 w-full overflow-hidden rounded-full bg-smb-surface-container-high border border-smb-outline-variant/40 mb-3">
        <div
          className={`h-full transition-all duration-500 ${overall > 50 ? 'bg-emerald-500' : overall >= 20 ? 'bg-amber-500' : 'bg-rose-500'}`}
          style={{ width: `${overall ?? 0}%` }}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-2.5 border-t border-smb-outline-variant/30">
        ...
        <div className="rounded-lg bg-smb-surface-container-low/60 p-2 border border-smb-outline-variant/30 flex flex-col justify-between">
          <div className="flex items-center justify-between gap-1 mb-1">
            ...
            {deviceIsCharging ? (
              <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-amber-500 bg-amber-500/15 px-1 py-0.2 rounded" title="Đang sạc">
          ...
          <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-smb-surface-container-high">
            <div
              className={`h-full transition-all duration-300 ${devBat > 50 ? 'bg-blue-500' : devBat >= 20 ? 'bg-amber-500' : 'bg-rose-500'}`}
