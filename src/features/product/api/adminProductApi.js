...
export const downloadProductImportTemplate = () =>
  client
    .get(`${ADMIN_ENDPOINT}/import-template`, { responseType: 'blob' })
    .then((res) => {
...
export const exportAdminProducts = () =>
  client
    .get(`${ADMIN_ENDPOINT}/export`, { responseType: 'blob' })
    .then((res) => {
      const url = window.URL.createObjectURL(new Blob([res.data]))
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', 'products_full_export.xlsx')
      document.body.appendChild(link)
      link.click()
      link.remove()
      window.URL.revokeObjectURL(url)
